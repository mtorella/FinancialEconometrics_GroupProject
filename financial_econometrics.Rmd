...
# Calculate cumulative return indices for each industry 
data$index_Cnsmr <- cumprod(1 + data$Cnsmr)
data$index_Manuf <- cumprod(1 + data$Manuf)
data$index_HiTec <- cumprod(1 + data$HiTec)
data$index_Hlth <- cumprod(1 + data$Hlth)

# Calculate the composite index as an average of the individual indices
data$Composite_Index <- rowMeans(data[, c("index_Cnsmr", "index_Manuf", "index_HiTec", "index_Hlth")])


# Use mutate() and lag() from dplyr to calculate the Momentum variable
data <- data %>%
  mutate(
    Momentum = (lag(Composite_Index, 1) / lag(Composite_Index, 13)) - 1
  )
...
```{r}
model_2 <- lm(Portfolio_Returns ~ Momentum, data = data)
summary(model_2)
```

```{r, warning=FALSE, message=FALSE}
ggplot(data, aes(x = Momentum, y = Portfolio_Returns)) +
  geom_point(color = "blue", alpha = 0.6) + 
  geom_smooth(method = "lm", color = "red", se = FALSE) +  
  labs(title = "Regression of Portfolio Returns on Price Ratio",
